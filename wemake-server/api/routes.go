...
func SetupRoutes(db *sqlx.DB, cfg *config.Config) *fiber.App {
	...
	boqHandler := handler.NewBOQHandler(boqService)
	profileHandler := handler.NewProfileHandler(profileService, cfg.PublicBaseURL, cld)
	...
	admin.Get("/platform-configs", middleware.RequireRole(authService, domain.RoleAccountManager, domain.RoleAdmin, domain.RoleSuperAdmin), platformConfigHandler.ListAll)
	admin.Post("/platform-configs", middleware.RequireRole(authService, domain.RoleSuperAdmin), platformConfigHandler.CreateConfig)
	admin.Delete("/platform-configs/:config_id", middleware.RequireRole(authService, domain.RoleSuperAdmin), platformConfigHandler.DeleteConfig)
	admin.Patch("/platform-configs/:config_id", middleware.RequireRole(authService, domain.RoleSuperAdmin), platformConfigHandler.UpdateConfig)
	...
	admin.Get("/factories/:factory_id/config", middleware.RequireRole(authService, domain.RoleAccountManager, domain.RoleAdmin, domain.RoleSuperAdmin), adminFactoryHandler.GetFactoryConfig)
	admin.Patch("/factories/:factory_id/config", middleware.RequireRole(authService, domain.RoleAdmin, domain.RoleSuperAdmin), adminFactoryHandler.AssignFactoryConfig)
	...
	factories.Get("/:factory_id/reviews/summary", reviewHandler.GetSummaryByFactory)
	...
	orders.Get("/:order_id/review", orderHandler.GetReviewState)
	orders.Post("/:order_id/review", orderHandler.CreateReview)
	...
	conversations.Post("/:conv_id/boq", boqHandler.Create)
	conversations.Post("/:conv_id/share-rfq", conversationHandler.ShareRFQ)
	...
	notifications.Delete("/:noti_id", notificationHandler.SoftDelete)
	...
	profile.Get("/summary", profileHandler.GetSummary)
	profile.Get("/transactions", profileHandler.ListTransactions)
	profile.Get("/reviews", profileHandler.ListMyReviews)
	profile.Get("/reviews/received", profileHandler.ListReceivedReviews)
	profile.Get("/notification-preferences", profileHandler.GetNotifPrefs)
	profile.Put("/notification-preferences", profileHandler.UpdateNotifPrefs)

	reviews := api.Group("/reviews")
	reviews.Put("/:review_id", reviewHandler.UpdateByUser)
	reviews.Delete("/:review_id", reviewHandler.DeleteByUser)
	...
	showcases.Post("/:showcase_id/images", showcaseHandler.CreateImage)
	...
	boqs := api.Group("/boq")
	boqs.Get("/", boqHandler.ListMine)
	boqs.Get("/:rfq_id", boqHandler.Get)
	boqs.Put("/:rfq_id", boqHandler.Update)
	boqs.Post("/:rfq_id/accept", boqHandler.Accept)
	boqs.Post("/:rfq_id/decline", boqHandler.Decline)
