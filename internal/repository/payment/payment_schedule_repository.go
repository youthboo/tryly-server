...
func (r *PaymentScheduleRepository) Create(s *domain.PaymentSchedule) error {
	...
	`, s.OrderID, s.InstallmentNo, s.DueDate, s.Amount, domain.PaymentScheduleStatusPending).
...
func (r *PaymentScheduleRepository) PatchStatus(scheduleID int64, status string) error {
	...
	if status == domain.PaymentScheduleStatusPaid {
		now := time.Now()
		paidAt = now
	}
	res, err := r.db.Exec(`
		...
		SET status = $1,
		    paid_at = CASE WHEN $1 = $4 THEN $2 ELSE paid_at END
		WHERE schedule_id = $3
	`, status, paidAt, scheduleID, domain.PaymentScheduleStatusPaid)
	if err != nil {
		return err
	}
...
func (r *PaymentScheduleRepository) PatchStatusByOrderAndInstallmentTx(tx *sqlx.Tx, orderID int64, installmentNo int, status string) error {
	...
	res, err := tx.Exec(`
		UPDATE payment_schedules
		SET status = $1,
		    paid_at = CASE WHEN $1 = $5 THEN $2 ELSE paid_at END
		WHERE order_id = $3 AND installment_no = $4
	`, status, paidAt, orderID, installmentNo, domain.PaymentScheduleStatusPaid)
	if err != nil {
		return err
	}
	n, _ := res.RowsAffected()
